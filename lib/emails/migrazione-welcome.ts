/**
 * Confirmation email sent to a lead who submits the "Migrazione a Shopify"
 * qualification form. The page is a candidatura (application), so the tone is
 * deliberately selective: we confirm receipt and say we'll reach out only if the
 * project is a fit — we do not promise a call to everyone.
 *
 * Email clients have limited CSS support, so the layout uses inline styles +
 * nested tables. The WhatsApp icon is an inlined SVG data URI (no hosted asset).
 * IT-only for now, mirroring the landing page.
 */
...
export function buildMigrazioneWelcomeEmail({
  name,
  waUrl,
}: {
  name: string
        ...
        <tr>
          <td style="padding:20px 36px 0;">
            <h1 style="margin:0;font-size:26px;font-weight:700;line-height:1.2;color:#121212;letter-spacing:-0.03em;">
              Ciao ${name}, abbiamo ricevuto la tua candidatura.
            </h1>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:16px 36px 0;">
            <p style="margin:0;font-size:16px;line-height:1.7;color:#444;">
              La esaminiamo personalmente. Se il progetto è in target, ti ricontattiamo
              per fissare una call e capire come migrare il tuo store su Shopify senza
              perdere vendite, clienti o posizioni su Google.
            </p>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:24px 36px 0;">
            <p style="margin:0 0 20px;font-size:15px;font-weight:600;color:#121212;">
              Vuoi aggiungere qualcosa? Scrivimi direttamente:
            </p>
